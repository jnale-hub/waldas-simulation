...
export const CartSummary: React.FC<CartSummaryProps> = ({
  ...
  return (
    <div
      className={`fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-[0_-5px_20px_rgba(0,0,0,0.1)] transition-transform duration-300 z-40 pb-safe ${totalSpent > 0 ? "translate-y-0" : "translate-y-0"}`}
    >
        ...
        <div className="flex items-center gap-3">
          <div className="hidden sm:block">
            <input
              ...
              checked
              readOnly
              className={`rounded ${theme.text} focus:ring-0`}
            />
            <span className="text-xs text-gray-500 ml-2">All</span>
          </div>
          <div className="flex flex-col">
            <span className="text-[10px] md:text-xs text-gray-500 font-medium text-right sm:text-left">
              Total Payment
            </span>
